...
<body class="is-preload">
			...
			<nav>
				<ul>
                  ...
                  function menu(){
					...
					<li>
						<select onchange="if(this.value) location.href=(this.value);" id="Adminpage" onclick="admin()">
		...
		<div id="main">
			...
			<article id="KOREA">
				...
				<div style="text-align: right;">
					<a href="#CHINA">중식&ensp;</a> <a href="#USA">양식&ensp;</a> 
					<a href="#JAPAN">일식</a>
				</div>
			...
			<article id="CHINA">
				...
				<div style="text-align: right;">
					<a href="#KOREA">한식&ensp;</a> <a href="#USA">양식&ensp;</a> <a href="#JAPAN">일식</a>
				</div>
			...
			<article id=kimchi class = "user">
				<form>
					...
					<div style="text-align: center;">
						<%if(vo!=null){ %>
						<input type="button" href="OutputSpice?REC_NUM=1&id=<%=vo.getid()%>" value="전송">
						<a href="OutputSpice?REC_NUM=1&id=<%=vo.getid()%>" >전송</a>
						<%-- <button onclick="OutputSpice?REC_NUM=1&id=<%=vo.getid()%>">전송</button> --%>
						<%} %>
					</div>
				</form>
			...
			<article id="ChangeInfo">
				...
				<form action="UpdateService" method="post">
					...
					<br>
					<h3>NICK</h3>
					<input name="nick" type="text" placeholder="변경할 NICK을 입력하세요">
					<br> <input type="submit" value="Change" class="button fit">
				</form>
				</span>
			</article>
			...
			<article id="Login">
				...
				<form action="LoginService" method="post">
					<h3>ID</h3>
					<input id="id" name="id" type="text" placeholder="ID를 입력하세요" required="required"> <br>
					<h3>PW</h3>
					<input id="pw" name="pw" type="password" placeholder="PW를 입력하세요" required="required"> <br>
					<div style="text-align: center;">
			...
			<article id="Product">
				...
				<form action="ProductService" method="post">
					...
					<br>
					<div style="text-align: center;">
						<input type="submit" value="저장" onclick="alert('제품 등록이 완료 됐습니다.')">
						<br>
					</div>
				</form>
				<br> <br>
			...
			<article id="MemberInfo">
				...
				<table>
					...
					<tr>
						...
						<td><%=mvo.getpw() %></td>
						<td><%=mvo.getnick() %></td>
						<td><a href="DeleteService?id=<%=mvo.getid()%>"
							onclick="if(!confirm('삭제 하시겠습니까?')){return false;}">삭제</a></td>
					</tr>
					<% } %>
				</table>
			...
			<article id="recipepost">
				...
				<form action="RecipePost" method="post">
					<h3>요리 명</h3>
					<input name="rec_name" type="text" placeholder="요리 이름 입력"
						required="required"> <br>
					<h3>소금</h3>
					<input name="salt" type="text" placeholder="소금 양 입력"
						required="required"> <br>
					<h3>설탕</h3>
					<input name="sugar" type="text" placeholder="설탕 양 입력"
						required="required"> <br>
					<h3>후추</h3>
					<input name="pepper" type="text" placeholder="후추 양 입력"
						required="required"> <br>
					<div style="text-align: center;">
						<input type="submit" value="작성"
							onclick="alert('성공적으로 레시피를 작성 하였습니다.')">
					</div>
				</form>
			</article>
	...
	<script>
      ...
      $("#user_input").on('click',function(){
  	 ...
  	 // 아두이노 연결(레시피의 값을)
     <%--  $("#Recipe_input").on('click',function(){
		let id = '<%=vo.getid()%>';
			$.ajax({
				url : "OutputSpice",
				type : "get",
				data : {"REC_NUM" : 1, "id" : id},
				dataType : "json",
				success : function(data){
					$('#result').html("전송 성공");
				},
